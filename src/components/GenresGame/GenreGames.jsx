...
const GenreGames = () => {
  ...
  useEffect(() => {
    const fetchGames = async () => {
      if (!genreName) {
        setError('No genre name provided');
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const genreGames = data.filter(game => 
          game.genre && game.genre.trim() === decodedGenreName
        );
        
        // Sort games alphabetically by title
        const sortedGames = genreGames.sort((a, b) => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white flex flex-col items-center justify-center px-4 py-12">
        <div className="text-2xl text-center">Loading games for {decodedGenreName}...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white px-4 py-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <button
            onClick={() => navigate(-1)}
            className="absolute left-4 top-4 bg-black bg-opacity-60 hover:bg-green-700 text-white rounded-full p-3 shadow-lg transition-all duration-200"
            aria-label="Back"
          >
          ...
          <p className="text-lg md:text-xl text-gray-300 mb-6">
            {games.length} games in this genre
          </p>
          ...
          <div className="max-w-md mx-auto mb-8">
            <input
              ...
              className="w-full px-4 py-3 bg-gray-800 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500"
            />
          </div>
                ...
                >
                  <div className="relative mb-3">
                    <img
                      src={game.thumbnail}
                      alt={game.title}
                      className="w-full h-32 object-cover rounded-lg border-2 border-green-600 group-hover:border-green-400 transition-colors duration-200"
                      onError={(e) => {
